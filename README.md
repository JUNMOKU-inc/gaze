...
- **LLM Focus Markers** — Pin / Rectangle を置くと、注釈入り画像とLLM向け補助テキストを同時生成
...
# 注目点を指定して注釈付き画像 + promptHint を出力
cargo run -p gaze-cli -- optimize screenshot.png --pin 640,320:broken button --rect 120,90,420,180:spacing issue
```
...
4. **必要ならマーク** — プレビューで Pin / Rectangle を置くと、注釈入り画像がクリップボードに再コピーされます
5. **ペースト** — `Cmd+V` で画像を貼り、必要に応じて生成された prompt hint も貼り付けます
...
- [ ] ウィンドウキャプチャ
- [ ] グローバルショートカットのカスタマイズ
- [x] LLM Focus Markers (Pin / Rectangle + prompt hint)
- [ ] 高度なアノテーションエディタ (矢印、テキスト、ブラー)
- [ ] OCRテキスト抽出モード
- [ ] バーストモード (連続キャプチャ)
- [ ] GIF / 短時間動画キャプチャ
