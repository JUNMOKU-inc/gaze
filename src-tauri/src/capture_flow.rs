...
#[cfg(target_os = "macos")]
pub fn execute_area_capture() -> Result<Option<CaptureMetadata>, String> {
...
/// Returns `Ok(None)` if the user cancelled (Escape).
#[cfg(target_os = "macos")]
pub fn execute_window_capture() -> Result<Option<CaptureMetadata>, String> {
    run_screencapture(&["-i", "-w", "-x"])
}

/// Execute fullscreen capture using macOS native `screencapture -x` (non-interactive).
#[cfg(target_os = "macos")]
pub fn execute_fullscreen_capture() -> Result<Option<CaptureMetadata>, String> {
    run_screencapture(&["-x"])
}

#[cfg(target_os = "macos")]
fn run_screencapture(args: &[&str]) -> Result<Option<CaptureMetadata>, String> {
    ...
    let processed = process_captured_file(&tmp_path);
    let _ = std::fs::remove_file(&tmp_path);
    processed.map(Some)
}

fn process_captured_file(file_path: &Path) -> Result<CaptureMetadata, String> {
    let raw_bytes =
